...
class NutritionService
{
    ...
    /**
     * Call the Spoonacular API and set nutrition field values on the entry.
     * Does NOT save the entry — the caller is responsible for that.
     *
     * @return array{success: bool, message: string}
     */
    public function calculate(Entry $entry): array
    {
        ...
        return ['success' => true, 'message' => 'Nutrition data calculated'];
    }

    /**
     * Calculate and save — convenience wrapper for CLI use.
     *
     * @return array{success: bool, message: string}
     */
    public function calculateAndSave(Entry $entry, bool $force = false): array
    {
        if (!$force && !$this->needsRecalculation($entry)) {
            return ['success' => true, 'message' => 'Skipped — ingredients unchanged'];
        }

        $result = $this->calculate($entry);
